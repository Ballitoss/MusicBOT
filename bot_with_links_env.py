...
RENDER VERSION - Gebruikt environment variable voor session
"""
import asyncio
import re
import logging
...
def restore_session_from_env():
    """Restore session file from environment variable"""
    session_b64 = os.getenv('TELETHON_SESSION')
    if not session_b64:
        logger.warning("⚠️ TELETHON_SESSION environment variable not set!")
        return False
    ...
    try:
        ...
        logger.info("✅ Session file restored from environment variable")
        return True
...
async def handle_telegram_link(update: Update, channel_id: int, message_id: int):
    ...
    try:
        if not telethon_client:
            await status_msg.edit_text("❌ Telethon client niet beschikbaar. Bot moet opnieuw gestart worden.")
            return
...
async def post_init(application: Application):
    ...
    if not restore_session_from_env():
        logger.error("❌ Kan session niet herstellen. Bot werkt niet!")
        return
    
    telethon_client = TelegramClient('user_session', API_ID, API_HASH)
    await telethon_client.start()
    logger.info("✅ Telethon verbonden met user session!")
